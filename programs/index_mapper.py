...
def normalizeSearchAttributes(attributes):
    return ''.join(str(attributes).split())
...
def main(separator='\t'):
    ...
    for line in sys.stdin:
        ...
        if last_visitorid != record['visitorid'] or \
           ...
           last_rawquery != record['rawquery'] or \
           last_searchattributes != normalizeSearchAttributes(record['searchattributes']):
